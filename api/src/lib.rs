...
/// provider alongside the `ring` that libsql already links: two compiled in leaves
...
/// Builds the app.
///
/// There is no separate development entrypoint any more. That one existed to apply
/// migrations at startup, which serverless would repeat on every cold start; the schema
/// is now applied once by `cargo run --bin migrate`.
pub async fn build(config: &Config) -> AppResult<Router> {
    install_crypto_provider();

    let db = Arc::new(db::Db::connect(&config.database_url, &config.database_token).await?);

    router(db, config)
}

fn router(db: Arc<db::Db>, config: &Config) -> AppResult<Router> {
    let state = AppState {
        db,
